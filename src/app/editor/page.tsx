...
import { Stage, Layer, Image as KonvaImage, Line, Text as KonvaText, Rect, Transformer } from 'react-konva';
import { motion } from 'motion/react';
import { Upload, RotateCw, Crop, Pen, Type, Undo2, Redo2, RotateCcw, Download } from 'lucide-react';
...
const useEditorStore = create<EditorState>((set, get) => ({
  ...
  saveState: () => {
    ...
    set((state) => ({
      undoStack: [...state.undoStack.slice(-4), snapshot],
      redoStack: [],
    }));
  },
  ...
  undo: () => {
    ...
    set({
      ...
      redoStack: [...state.redoStack.slice(-4), currentSnapshot],
    });
  },
  ...
  redo: () => {
    ...
    set({
      ...
      undoStack: [...state.undoStack.slice(-4), currentSnapshot],
    });
  },
...
// ============ TOOLBAR COMPONENT ============
const Toolbar = () => {
  const { currentTool, setCurrentTool, undo, redo, reset, undoStack, redoStack, saveState, rotation, setRotation } = useEditorStore();
...
const InfoConsole = () => {
  const { cursorPos, imageSize, rotation, scale } = useEditorStore();
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl px-8 py-4 shadow-2xl flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 bg-gradient-to-br from-blue-400 to-purple-500 rounded-xl flex items-center justify-center">
              <Crop className="text-white" size={20} />
            </div>
            <div>
              <h1 className="text-white text-xl font-bold">Image Editor</h1>
          ...
          <motion.button
            ...
            className="bg-gradient-to-r from-blue-500 to-purple-600 text-white px-6 py-2.5 rounded-xl font-medium flex items-center gap-2 shadow-lg hover:shadow-xl transition-shadow"
          >
            <Download size={18} />
            Download
          </motion.button>
